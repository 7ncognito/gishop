...
$order_query = "SELECT * FROM customer_order WHERE customer_id=$customer_id AND order_status != 'cancelled'";
    ...
    <table class="table table-responsive table-hover ">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><img src="../img/<?php echo $img1;?>" width="100%"></td>
                <td><h6><?php echo $title;?></h6></td>
...
if($order_status != 'delivered'){
    echo '<button class="btn btn-danger" onclick="openCancelForm(\''.$order_invoice.'\')">Cancel Order</button>';
} else {
...
<?php                     
    ...
    echo
 "<h2 class='text-center text-secondary mt-5 mb-5'>You haven't ordred anything yet </h2>";
              }
              ?>
     ...
     <script type="text/javascript">
       function openCancelForm(invoice) {
    document.getElementById('cancel-form').style.display = 'block';
    document.getElementById('invoice-no').value = invoice;
}

     </script>
     <div id="cancel-form" class="modal">
    <form class="modal-content animate" action="cancel-order.php" method="post">
        <div class="imgcontainer">
            <span onclick="document.getElementById('cancel-form').style.display='none'" class="close" title="Close">&times;</span>
            <h2>Cancel Order</h2>
        </div>
        <div class="container">
            <label for="invoice-no"><b>Invoice Number</b></label>
            <input type="text" placeholder="Enter Invoice Number" name="invoice-no" id="invoice-no" readonly>

            <label for="reason"><b>Reason for Cancellation</b></label>
            <textarea placeholder="Enter Reason" name="reason" required></textarea>
